...
async def ver_perfil():
    # Datos fijos directos para evitar que Render truene con SQLite
    nombre_db = "Dante"
    raza_db = "Chihuahua • Macho • Marrón con Blanco"
    ...
    salud_db = "Condición estable, gordo y sano. Está castrado (esterilizado)."
    direccion_db = "Anenecuilco No. 11, Col. Tierra y Libertad."
    tel_principal_db = "627-279-2334 (Hermana)"
    tel_secundario_db = "627-131-0481 (Aby)"
    ...
    <head>
        ...
        <title>AlertaPata - Perfil de {nombre_db}</title>
        <style>
    ...
    <body>
        <div class="card">
            ...
            <img src="{foto_perro}" alt="Foto de {nombre_db}" class="pet-img">
            <h1>{nombre_db}</h1>
            <span class="breed">{raza_db}</span>
            
            <div class="info-section">
                <p><strong>Comportamiento:</strong> {comportamiento_db}</p>
                <p><strong>Salud:</strong> {salud_db}</p>
                <p><strong>Dirección de Casa:</strong> {direccion_db}</p>
                <p><strong>Contactos de Emergencia:</strong><br>• Tel Principal: {tel_principal_db}<br>• Tel Secundario: {tel_secundario_db}</p>
            </div>
